...
namespace AdventOfCode2020.Tests
{
    public class Day04
    {
        ...
        public async Task ProblemOne_ReturnsExpectedResult_WhenEnteringSampleFromWebsite(string input, bool isValid)
        {
            var output = await Solution04.ProblemOneAsync(new []{ new[] { input } });
            Assert.Equal(isValid ? 1 : 0, output);
        }
        ...
        public async Task ProblemTwo_ReturnsExpectedResult_WhenEnteringSampleFromWebsite(string input, bool isValid)
        {
            var output = await Solution04.ProblemTwoAsync(new[] { new[] { input } });
            Assert.Equal(isValid ? 1 : 0, output);
        }
    }
}
